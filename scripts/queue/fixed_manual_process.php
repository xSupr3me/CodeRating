...
// Définir le chemin absolu vers le dossier coursero
$coursero_dir = '/var/www/coursero';

// Inclure le fichier config.php
require_once $coursero_dir . '/config.php';

// Inclure le correcteur si nécessaire
if (file_exists(dirname(__DIR__) . '/correction/corrector.php')) {
    require_once dirname(__DIR__) . '/correction/corrector.php';
} else {
    // Simulation basique du correcteur si le fichier n'est pas trouvé
    class Corrector {
        private $submission;
        ...
        public function run() {
            // Simuler un score aléatoire entre 0 et 100
            $score = mt_rand(0, 100);
            return [
                'success' => true,
                'score' => $score,
                'details' => [
                    1 => ['passed' => ($score > 50), 'expected' => 'Expected output', 'output' => 'Actual output']
                ]
            ];
        }
    }
